...
class Employee
{
    ...
    public function getInfo(PDO $db)
    {
        $stmt = $db->prepare('SELECT e.id, e.first_name, e.middle_name, e.last_name, e.department,
             e.position, e.passportID, em.email_type, em.email, ph.phone_type, ph.phone, c.country_code
            FROM employees AS e
    ...
    /**
     * @param PDO $db
     * @param string $phoneNewNumbr
     * @param string $phoneType
     */
    public function updatePhone(PDO $db, string $phoneType, string $oldPhoneNumber, string $phoneNewNumbr)
    {
        $stmt = $db->prepare('UPDATE employee_phones AS ph
     ...
     */
    public function updateEmail(PDO $db, string $emailType, string $oldEmail, string $emailNew)
    {
        $stmt = $db->prepare('UPDATE employee_emails AS em
            INNER JOIN employees AS e
                USING (id)
            SET em.email = ?
            WHERE e.first_name = ?
